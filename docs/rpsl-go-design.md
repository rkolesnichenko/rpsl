...
// Validate checks an object against a class/attribute profile (RIPE, IRRd, RFCStrict).
...
- **RIPE deviations.** RIPE's RPSL diverged from RFC 2622 over 25 years (extra attributes, dropped features, `auth:` formats, `abuse-c:`). The class/attribute dictionary is data-driven (a loadable table), with three built-in profiles. The RIPE profile is RIPE's own templates (`whois -t <class>`, kept in `object/testdata/ripe-templates` and checked by `TestRIPEProfileMatchesTemplates`, and against whois.ripe.net by the live test). The IRRd profile is IRRd 4's class tables — what RADB and the IRRs it mirrors accept — read from IRRd's `rpsl_objects.py` (kept in `object/testdata/irrd` at a release tag, checked by `TestIRRdProfileMatchesSource`, and against IRRd's latest release by a live test); it differs from RIPE's where the registries do (`mnt-by:` optional on aut-num, `rev-srv:`, `geoidx:`, `changed:` still allowed). The RFC-strict profile follows the tables of RFC 2622, 2725, 2726 and 4012.
