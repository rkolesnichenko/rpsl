...
func TestDiagnosticRulesAreDocumented(t *testing.T) {
	...
	catalog := []struct {
		...
		{"object/route-set-members-no-length", obj("route-set: RS-X\nmembers: 192.0.2.1\n")},
		{"object/route-set-mp-members-no-length", obj("route-set: RS-X\nmp-members: 2001:db8::1\n")},
		...
		{"object/route-abbreviated-prefix", obj("route: 143.208.148/22\norigin: AS1\n")},
		...
		{"policy/abbreviated-prefix", imp("from AS1 accept {143.208.148/22}")},
		{"policy/unicode-space", imp("from AS1\u00a0accept ANY")},
		{"policy/range-op", imp("from AS1 accept AS-X^+24")},
		{"policy/range-op-empty", imp("from AS1 accept {1.0.0.0/8}^64")},
		...
		{"policy/ifaddr", pol(parseIfaddr, "fe80::1%eth0 masklen 64")},
		{"policy/interface", pol(parseIface, "fe80::1%eth0 masklen 64")},
		{"policy/router", imp("from AS1 fe80::1%eth0 accept ANY")},
